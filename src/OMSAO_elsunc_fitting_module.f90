...
MODULE OMSAO_elsunc_fitting_module
  ...
  REAL (KIND=r8), PARAMETER  :: zero = 0.0_r8, one = 1.0_r8
  ...
  REAL (KIND=r8), SAVE  :: betkm2,d1km2,fsqkm2,dxnkm2,alfkm2,aupkm2,betkm1,d1km1,  &
  ...
  REAL (KIND=r8), SAVE  :: philat,bestpg
...
CONTAINS
  ...
  SUBROUTINE elsunc(x,n,mdc,m,ffunc,bnd,bl,bu, p,w, EXIT,f,c)
    ...
    REAL (KIND=r8), INTENT(IN OUT)  :: x(:) !x(1:n)!
    INTEGER, INTENT(IN)        :: n
    INTEGER, INTENT(IN)        :: mdc
    INTEGER, INTENT(IN)        :: m
    INTEGER, INTENT(IN)        :: bnd
    REAL (KIND=r8), INTENT(IN OUT)  :: bl(:) !bl(1:n)!
    REAL (KIND=r8), INTENT(IN OUT)  :: bu(:) !bu(1:n) !
    !tpk INTEGER, INTENT(OUT)       :: p(11)
    INTEGER, INTENT(INOUT)       :: p(11) !tpk
    !tpk REAL (KIND=r8), INTENT(OUT)     :: w(6)
    REAL (KIND=r8), INTENT(INOUT)     :: w(6)  !tpk
    INTEGER, INTENT(OUT)       :: EXIT
    REAL (KIND=r8), INTENT(OUT)     :: f(:) !f(1:m)!
    REAL (KIND=r8), INTENT(OUT)     ::  c(:,:) !c(mdc,n)!
    ...
    !        (MDC>=M) CONTAINING THE MAIN PART OF THE COVARIANCE MATRIX
    !            T   -1
    ...
    REAL (KIND=r8) :: rootsp,bl1,bu1
    LOGICAL   :: sec
    REAL (KIND=r8), PARAMETER  :: big = 1.0E32_r8
    ...
    IF(w(2) < zero) w(2)=10.0_r8*rootsp
    IF(w(3) < zero) w(3)=srelpr
    IF(w(4) < zero) w(4)=10.0_r8*rootsp
...
1000 FORMAT(//t21, 31('*')/ t21, '* pseudo-rank of the jacobian *'/  &
  ...
  SUBROUTINE covar(n, r, ipvt, tol, wa)
    ...
    INTEGER, INTENT(IN)     :: n
    REAL (KIND=r8), INTENT(OUT)  :: r(:,:)   ! r(ldr,n)
  ...
  SUBROUTINE lsunc(x,n,mdc,mdg,m,tol,epsrel,epsabs,epsx,sec,  &
    ...
    INTEGER, INTENT(IN)        :: m
    REAL (KIND=r8), INTENT(IN)      :: tol
    REAL (KIND=r8), INTENT(IN)      :: epsrel
    REAL (KIND=r8), INTENT(IN)      :: epsabs
    REAL (KIND=r8), INTENT(IN)      :: epsx
    ...
    REAL (KIND=r8) :: tau,fsum,xnorm,d1sqs,beta,alpha,gnorm,dxnorm,alphup
    REAL (KIND=r8) :: alfnoi,prekm1,xdiff
    LOGICAL   :: restar
    REAL (KIND=r8), SAVE :: c1 = 0.0_r8
...
50  EXIT=error
  ...
  SUBROUTINE soliuc(x,n,f,m,ffunc,tau,scale,mdc,bl,bu,aset,nract,c,funcev,  &
    ...
    INTEGER, INTENT(IN OUT)    :: p(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: diag(:)
    REAL (KIND=r8), INTENT(OUT)     :: g(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: qraux(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: d1sqs
    REAL (KIND=r8), INTENT(OUT)     :: beta
    REAL (KIND=r8), INTENT(OUT)     :: prekm1
    REAL (KIND=r8), INTENT(OUT)     :: gnorm
    INTEGER, INTENT(IN OUT)    :: prank
    ...
    REAL (KIND=r8)  :: work(n), w1(m), w2(m), work2(n,1)
...
20  ERR=ctrl
  ...
  SUBROUTINE scaunc(scale,c,m,n,cnorm,diag)
    ...
    INTEGER, INTENT(IN)        :: m
    INTEGER, INTENT(IN)        :: n
    REAL (KIND=r8), INTENT(OUT)     :: cnorm
    REAL (KIND=r8), INTENT(OUT)     :: diag(:)
...
30  prank=k
  ...
  SUBROUTINE gndunc(f,m,c,n,scale,diag,p,qraux,prank,dx,d1sqs,v,work,y)
    ...
    REAL (KIND=r8), INTENT(OUT)     :: dx(:)
    REAL (KIND=r8), INTENT(OUT)     :: d1sqs
    REAL (KIND=r8), INTENT(IN OUT)  :: v(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: work(:)
...
40  RETURN
  ...
  SUBROUTINE jacdif(x,n,f,m,ffunc,c,mdc,ERR,w1)
    ...
    INTEGER   :: i,j,ctrl
    REAL (KIND=r8) :: delta,xtemp,deltaj
...
30  ERR=ctrl
  ...
  SUBROUTINE analuc(k,restar,kod,fsum,d1sqs,beta,dxnorm,prekm1,  &
    ...
    REAL (KIND=r8), INTENT(IN OUT)  :: fsum
    REAL (KIND=r8), INTENT(IN OUT)  :: d1sqs
    REAL (KIND=r8), INTENT(IN OUT)  :: beta
    REAL (KIND=r8), INTENT(OUT)     :: dxnorm
    REAL (KIND=r8), INTENT(IN OUT)  :: prekm1
    REAL (KIND=r8), INTENT(IN OUT)  :: x(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: dx(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: diag(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: qraux(:)
...
50  restar=.false.
  ...
  SUBROUTINE gnavuc(k,restar,kod,secind,fsum,beta,prekm1,nract,aset,gndok)
    ...
    REAL (KIND=r8), INTENT(IN)    :: fsum
    REAL (KIND=r8), INTENT(IN)    :: beta
    REAL (KIND=r8), INTENT(IN)    :: prekm1
    ...
    REAL (KIND=r8) :: pgress,fnorm,ac
    REAL (KIND=r8), PARAMETER  :: c1 = 0.5_r8, c2 = 0.1_r8, c3 = 4.0_r8,  &
         c4 = 10.0_r8, c5 = 0.016_r8
    ...
    ac=MIN(one,aupkm1)
    IF(pgress > c2*ac*(2.0_r8-ac)*d1km1 .AND. fnorm < c3*beta) return
...
30  secind=kod
  ...
  SUBROUTINE dimsub(restar,kod,fsum,f,m,c,n,  &
    ...
    REAL (KIND=r8), INTENT(IN)      :: f(:)
    INTEGER, INTENT(IN)        :: m
    REAL (KIND=r8), INTENT(IN OUT)  :: c(:,:)   ! c(mdc,n)
    INTEGER, INTENT(IN)        :: n
    INTEGER, INTENT(IN)        :: scale
    REAL (KIND=r8), INTENT(IN)      :: diag(:)
    INTEGER, INTENT(IN)        :: p(:)
    ...
    REAL (KIND=r8) :: bn,sn,dummy(1),pgress,work(n),w1(m)
    ...
    !60  IF(icount > 1) GO TO 80
    !70  bestpg=0.5_r8*fsum - philat
    !    irank=lattry
    !80  pgress=0.5_r8*fsum - philat
    !    PRINT *, pgress, bestpg, philat
    !    IF(pgress > bestpg) GO TO 70
    !    rank=lattry-1
    !    IF(lattry <= 1) THEN
    !       rank=irank
    !       iter=iter+1
    ...
    pgress=0.5_r8*fsum - philat
    IF ( icount <= 1 .OR. pgress > bestpg ) THEN
       bestpg=0.5_r8*fsum - philat
       irank=lattry
    END IF
    rank=lattry-1
    IF(lattry <= 1) THEN
...
100 RETURN
  ...
  SUBROUTINE pregn(s,sn,b,bn,mindim,prank,dim)
    ...
    REAL (KIND=r8), INTENT(IN OUT)  :: s(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: sn
    REAL (KIND=r8), INTENT(IN OUT)  :: b(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: bn
    INTEGER, INTENT(IN)        :: mindim
    INTEGER, INTENT(IN)        :: prank
    INTEGER, INTENT(OUT)       :: dim

    REAL (KIND=r8), SAVE  :: smax = 0.2_r8, rmin = 0.5_r8
    INTEGER          :: i, k, m1
...
20  dim=k
  ...
  SUBROUTINE presub(s,b,bn,rabs,prank,fsum,dim)
    ...
    REAL (KIND=r8), INTENT(IN)  :: bn
    REAL (KIND=r8), INTENT(IN)  :: rabs
    INTEGER, INTENT(IN)    :: prank
    REAL (KIND=r8), INTENT(IN)  :: fsum
    INTEGER, INTENT(OUT)   :: dim
    ...
    REAL (KIND=r8), SAVE  :: stepb = 0.2_r8, pgb1 = 0.3_r8, pgb2 = 0.1_r8,  &
         predb = 0.7_r8, rlenb = 2.0_r8
    INTEGER          :: i, i1
    REAL (KIND=r8)        :: pgress
...
10  dim=rngkm1
  ...
  SUBROUTINE secuc(ffunc,x,dx,diag,p,qraux,n,scale,f,v,c,mdc,m,mdg,nract,  &
    ...
    REAL (KIND=r8)  :: w1(m), w3(n), gmat(n,n)
...
130 RETURN
  ...
  SUBROUTINE hess(ffunc,b,x,n,v,f1,f2,f3,f4,m,error)
    ...
    REAL (KIND=r8), INTENT(OUT)     :: f1(:)
    REAL (KIND=r8), INTENT(OUT)     :: f2(:)
    REAL (KIND=r8), INTENT(OUT)     :: f3(:)
    REAL (KIND=r8), INTENT(OUT)     :: f4(:)
    INTEGER, INTENT(IN)        :: m
    INTEGER, INTENT(OUT)       :: error
    ...
    INTEGER   :: j,k,l,ctrl
    REAL (KIND=r8) :: athird,eps1,eps2,epsk,epsj,xk,xj,term,dummy(1,1),sum
    ...
    DO  k=1,n
       ...
       DO  j=1,k
          ...
          DO  l=1,m
             ...
             sum=sum + term/(4.0_r8*epsk*epsj)*v(l)
          END DO
          b(k,j)=sum
          IF(k == j) CYCLE
          b(j,k)=sum
...
40  error=ctrl
  ...
  SUBROUTINE termuc(prank,error,restar,maxit,k,fsum,d1sqs,n,xnorm,  &
    ...
    INTEGER, INTENT(IN)        :: k
    REAL (KIND=r8), INTENT(IN)      :: fsum
    REAL (KIND=r8), INTENT(IN)      :: d1sqs
    INTEGER, INTENT(IN)        :: n
    REAL (KIND=r8), INTENT(IN)      :: xnorm
    REAL (KIND=r8), INTENT(IN OUT)  :: alfnoi
    REAL (KIND=r8), INTENT(IN)      :: xdiff
    REAL (KIND=r8), INTENT(IN)      :: epsabs
    REAL (KIND=r8), INTENT(IN)      :: epsrel
    REAL (KIND=r8), INTENT(IN)      :: epsx
    REAL (KIND=r8), INTENT(IN OUT)  :: g(:)
    ...
    REAL (KIND=r8)        :: rlmax
    INTEGER          :: i
    REAL (KIND=r8), SAVE  :: noise = 0.1_r8, eps = 0.01_r8, epsl = 0.001_r8
...
60  IF(k > maxit) EXIT=-2
  ...
  SUBROUTINE stepuc(x,g,dx,n,f,v1,m,phi,ffunc,kod,prank,bl,bu,  &
    ...
    INTEGER, INTENT(IN)        :: n
    REAL (KIND=r8), INTENT(OUT)     :: f(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: v1(:)
    INTEGER, INTENT(IN)        :: m
    !tpk REAL (KIND=r8), INTENT(OUT)     :: phi
    REAL (KIND=r8), INTENT(INOUT)     :: phi !tpk
    INTEGER, INTENT(IN OUT)    :: kod
    INTEGER, INTENT(IN)        :: prank
    REAL (KIND=r8), INTENT(IN)      :: bl(:)
    REAL (KIND=r8), INTENT(IN)      :: bu(:)
    ...
    REAL (KIND=r8), INTENT(OUT)     :: alphup
    REAL (KIND=r8), INTENT(OUT)     :: xdiff
    ...
    INTEGER   :: i,ctrl,iev
    REAL (KIND=r8) :: alplow,phikp1,xel,dummy(1,1),magfy,dphize
    ...
    alphup=3.0_r8
    !************************
    IF(ifree > 0 .AND. indic < 0) alphup=10.0_r8*alphup
    !************************
    IF(bnd /= 0)THEN
...
40  alpha=MIN(one,alphup)
    ...
    IF(-1 == ctrl) phi=phi*2.0_r8
    IF(ctrl == 1) phi=0.5_r8*dnrm2(m,f,1)**2
...
60  RETURN
  ...
  SUBROUTINE evreuc(xkm1,n,m,k,ffunc,funcev,alpha,alphup,d1sqs,beta,fsum,  &
    ...
    !tpk INTEGER, INTENT(OUT)       :: funcev
    INTEGER, INTENT(INOUT)       :: funcev  !tpk
    REAL (KIND=r8), INTENT(IN OUT)  :: alpha
    REAL (KIND=r8), INTENT(IN OUT)  :: alphup
    REAL (KIND=r8), INTENT(IN)      :: d1sqs
    REAL (KIND=r8), INTENT(IN)      :: beta
    REAL (KIND=r8), INTENT(IN)      :: fsum
    REAL (KIND=r8), INTENT(IN)      :: dxnorm
    INTEGER, INTENT(IN)        :: kod
    INTEGER, INTENT(IN)        :: prank
    REAL (KIND=r8), INTENT(IN OUT)  :: phi
    INTEGER, INTENT(IN OUT)    :: error
    REAL (KIND=r8), INTENT(IN OUT)  :: x(:)
    REAL (KIND=r8), INTENT(OUT)     :: f(:)
    LOGICAL, INTENT(OUT)       :: restar
    ...
    INTEGER   :: i,ctrl
    REAL (KIND=r8) :: c2,dummy(1,1),dsqrel
...
40  error=ctrl
  ...
  SUBROUTINE reavuc(error,alpha,alphup,fsum,phi,restar)
    ...
    IF(-2 == error) GO TO 10
    IF(phi < 0.5_r8*fsum) GO TO 10

5   error=-5
    RETURN

10  IF(alpha <= alphup/3000.0_r8) restar=.true.
  ...
  SUBROUTINE outuc (iprint, k, restar, UNIT, phi, gnorm, eval, aset,  &
    ...
    reduc=fsqkm1 - 2.0_r8*phi
    ac=MIN(one,aupkm1)
    pred=ac*(2.0_r8-ac)*d1km1
...
30  FORMAT ('', (t60, 5I3))
  ...
  SUBROUTINE grad(a,m,n,f,g)
    ...
    REAL (KIND=r8), INTENT(IN)   :: a(:,:)   ! a(mda,n)
    INTEGER, INTENT(IN)     :: m
    INTEGER, INTENT(IN)     :: n
    REAL (KIND=r8), INTENT(IN)   :: f(:)
    REAL (KIND=r8), INTENT(OUT)  :: g(:)
  ...
  SUBROUTINE releps(srelpr)
    ...
    REAL (KIND=r8), INTENT(OUT)  :: srelpr
  ...
  SUBROUTINE lineuc(xold,g,p,f,v1,m,n,alpha,phizer,dphize,alflow,  &
    ...
    REAL (KIND=r8), INTENT(IN OUT)  :: xold(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: g(:)
    ...
    INTEGER, INTENT(IN)        :: m
    INTEGER, INTENT(IN)        :: n
    !tpk REAL (KIND=r8), INTENT(OUT)     :: alpha
    REAL (KIND=r8), INTENT(INOUT)     :: alpha !tpk
    REAL (KIND=r8), INTENT(IN)      :: phizer
    REAL (KIND=r8), INTENT(IN)      :: dphize
    REAL (KIND=r8), INTENT(IN)      :: alflow
    REAL (KIND=r8), INTENT(IN)      :: alfupp
    REAL (KIND=r8), INTENT(OUT)     :: phialf
    REAL (KIND=r8), INTENT(OUT)     :: xdiff
    INTEGER, INTENT(OUT)       :: eval
    INTEGER, INTENT(OUT)       :: EXIT
    ...
    INTEGER   :: ctrl,i,k
    REAL (KIND=r8) :: eta,tau,gamma,alfmax,alfmin,alfk,alfkm1,alfkm2,alfkp1,  &
         pmax,beta,diff,pbeta,phik,phikm1,phikm2,pk,xmin
    REAL (KIND=r8) :: fnew(m),v2(m)
    LOGICAL   :: reduce
...
1030 EXIT=k
  ...
  SUBROUTINE linuc2(m,v1,fnew,f,alfk,v2)
    ...
    INTEGER, INTENT(IN)     :: m
    REAL (KIND=r8), INTENT(IN)   :: v1(:)
    REAL (KIND=r8), INTENT(IN)   :: fnew(:)
    REAL (KIND=r8), INTENT(IN)   :: f(:)
    REAL (KIND=r8), INTENT(IN)   :: alfk
    REAL (KIND=r8), INTENT(OUT)  :: v2(:)

    REAL (KIND=r8) :: f1,f2,f3,alf
    INTEGER   :: i
  ...
  SUBROUTINE reduc(alf,phialf,alfk,pk,diff,eta,xold,  &
    ...
    REAL (KIND=r8), INTENT(OUT)     :: alf
    REAL (KIND=r8), INTENT(IN OUT)  :: phialf
    REAL (KIND=r8), INTENT(IN)      :: alfk
    REAL (KIND=r8), INTENT(IN)      :: pk
    REAL (KIND=r8), INTENT(IN OUT)  :: diff
    REAL (KIND=r8), INTENT(IN)      :: eta
    REAL (KIND=r8), INTENT(IN OUT)  :: xold(:)
    REAL (KIND=r8), INTENT(IN)      :: p(:)
    REAL (KIND=r8), INTENT(OUT)     :: f(:)
    REAL (KIND=r8), INTENT(OUT)     :: xnew(:)
    REAL (KIND=r8), INTENT(OUT)     :: fnew(:)
    INTEGER, INTENT(IN)        :: m
    INTEGER, INTENT(IN)        :: n
    INTEGER, INTENT(OUT)       :: k
    REAL (KIND=r8), INTENT(OUT)     :: phik
    ...
    INTEGER    :: ctrl
    REAL (KIND=r8)  :: c1
    REAL (KIND=r8), SAVE  :: delta = 0.2_r8
...
40  reduce=.false.
  ...
  SUBROUTINE gauc(xold,p,f,m,n,ffunc,k,alfmin,EXIT,xnew,  &
    ...
    REAL (KIND=r8), INTENT(OUT)     :: phiu
    REAL (KIND=r8), INTENT(IN)      :: tau
    REAL (KIND=r8), INTENT(IN OUT)  :: pmax
    !tpk REAL (KIND=r8), INTENT(IN OUT)  :: srelpr

    EXTERNAL ffunc
    
    REAL (KIND=r8)  :: phix, sqreps, x
    INTEGER    :: ctrl
...
20  u=x
  ...
  SUBROUTINE chder(dphize,phizer,xold,p,m,n,ffunc,k,EXIT,alfk,phik)
    ...
    INTEGER   :: ctrl
    REAL (KIND=r8) :: phimk,dphifo,dphiba,dphice,maxdif,xnew(n),fnew(m)
...
20  EXIT=-1
  ...
  SUBROUTINE quamin(x,fx,w,fw,v,fv,u)
    ...
    REAL (KIND=r8), INTENT(IN)   :: x
    REAL (KIND=r8), INTENT(IN)   :: fx
    REAL (KIND=r8), INTENT(IN)   :: w
    REAL (KIND=r8), INTENT(IN)   :: fw
    REAL (KIND=r8), INTENT(IN)   :: v
    REAL (KIND=r8), INTENT(IN)   :: fv
    REAL (KIND=r8), INTENT(OUT)  :: u

    ! Internal variables
    REAL (KIND=r8)  :: d1, d2, s, q
  ...
  SUBROUTINE minrn(x,fx,w,fw,v,fv,alfmin,alfmax,pmax,srelpr,u,pu)
    ...
    REAL (KIND=r8), INTENT(IN)      :: x
    REAL (KIND=r8), INTENT(IN)      :: fx
    REAL (KIND=r8), INTENT(IN OUT)  :: w
    REAL (KIND=r8), INTENT(IN)      :: fw
    REAL (KIND=r8), INTENT(IN OUT)  :: v
    REAL (KIND=r8), INTENT(IN)      :: fv
    REAL (KIND=r8), INTENT(IN OUT)  :: alfmin
    REAL (KIND=r8), INTENT(IN OUT)  :: alfmax
    REAL (KIND=r8), INTENT(IN)      :: pmax
    REAL (KIND=r8), INTENT(IN OUT)  :: srelpr
    REAL (KIND=r8), INTENT(OUT)     :: u
    REAL (KIND=r8), INTENT(OUT)     :: pu

    ! Internal variables
    REAL (KIND=r8)  :: eps, t1, t2, t3
  ...
  SUBROUTINE minrm(v0,v1,v2,m,alfmin,alfmax,xmin,x,px,y,py)
    ...
    REAL (KIND=r8), INTENT(IN OUT)  :: v0(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: v1(:)
    REAL (KIND=r8), INTENT(IN OUT)  :: v2(:)
    INTEGER, INTENT(IN)        :: m
    REAL (KIND=r8), INTENT(IN)      :: alfmin
    REAL (KIND=r8), INTENT(IN)      :: alfmax
    REAL (KIND=r8), INTENT(IN)      :: xmin
    REAL (KIND=r8), INTENT(IN OUT)  :: x
    REAL (KIND=r8), INTENT(OUT)     :: px
    REAL (KIND=r8), INTENT(OUT)     :: y
    REAL (KIND=r8), INTENT(OUT)     :: py
    ...
    INTEGER   :: k
    REAL (KIND=r8) :: v0norm,v1norm,v2norm,scv0v1,scv0v2,scv1v2,pprim,pbiss,  &
         d,error,h0,dm,hm,p,q,delta,a1div3,x0,x1,x2,x3
    REAL (KIND=r8), SAVE  :: eps = 1.E-04_r8
    ...
    pbiss=xmin**2*v2norm*v2norm*6.0_r8 + xmin*scv1v2*6.0_r8 + 2.0_r8*scv0v2 + &
         v1norm**2
    h0=ABS(pprim/pbiss)
    dm=ABS(6.d0*scv1v2 + xmin*v2norm*v2norm*12.0_r8) + h0*v2norm*v2norm*24.0d0
...
50  pprim=pol3(scv0v1,scv0v2,v1norm,scv1v2,v2norm,x0)
    pbiss=x0**2*v2norm*v2norm*6.0_r8 + x0*scv1v2*6.0_r8 + 2.0_r8*scv0v2 + v1norm**2
    d=-pprim/pbiss
    x=x0 + d
...
100 x=MIN(x,alfmax)
  ...
  SUBROUTINE minrm1(v0,v1,v2,m,v0norm,v1norm,v2norm,scv0v1, scv0v2,scv1v2)
    ...
    REAL (KIND=r8), INTENT(IN OUT)  :: v2(:)
    INTEGER, INTENT(IN)        :: m
    REAL (KIND=r8), INTENT(OUT)     :: v0norm
    REAL (KIND=r8), INTENT(OUT)     :: v1norm
    REAL (KIND=r8), INTENT(OUT)     :: v2norm
    REAL (KIND=r8), INTENT(OUT)     :: scv0v1
    REAL (KIND=r8), INTENT(OUT)     :: scv0v2
    REAL (KIND=r8), INTENT(OUT)     :: scv1v2

    ! Internal variables
    REAL (KIND=r8)  :: sc1, sc2, sc3
    INTEGER    :: i
  ...
  SUBROUTINE minrm2(v1nrm,v2nrm,scv0v1,scv0v2,scv1v2,p,q,delta,a1div3)
    ...
    REAL (KIND=r8), INTENT(IN)   :: v1nrm
    REAL (KIND=r8), INTENT(IN)   :: v2nrm
    REAL (KIND=r8), INTENT(IN)   :: scv0v1
    REAL (KIND=r8), INTENT(IN)   :: scv0v2
    REAL (KIND=r8), INTENT(IN)   :: scv1v2
    REAL (KIND=r8), INTENT(OUT)  :: p
    REAL (KIND=r8), INTENT(OUT)  :: q
    REAL (KIND=r8), INTENT(OUT)  :: delta
    REAL (KIND=r8), INTENT(OUT)  :: a1div3
    ...
    ! Internal variables
    REAL (KIND=r8)  :: a1, a2, a3

    a1=1.5_r8*scv1v2/v2nrm/v2nrm
    a2=0.5_r8*((v1nrm/v2nrm)**2 + scv0v2/v2nrm/v2nrm*2.0_r8)
    a3=0.5_r8*scv0v1/v2nrm/v2nrm
    p=a2 - a1*a1/3._r8
    q=a3-a1*a2/3.d0 + 2.d0*a1*a1*a1/27.0_r8
  ...
  SUBROUTINE oner(q,delta,a1div3,x)
    ...
    REAL (KIND=r8), INTENT(IN)   :: a1div3
    REAL (KIND=r8), INTENT(OUT)  :: x

    ! Internal variables
    REAL (KIND=r8)  :: arg1, arg2, a3rd, sqd, s1, s2, t
  ...
  SUBROUTINE twor(p,q,delta,a1div3,x1,x2,x3)
    ...
    REAL (KIND=r8), INTENT(IN)   :: delta
    REAL (KIND=r8), INTENT(IN)   :: a1div3
    REAL (KIND=r8), INTENT(OUT)  :: x1
    REAL (KIND=r8), INTENT(OUT)  :: x2
    REAL (KIND=r8), INTENT(OUT)  :: x3
    ...
    REAL (KIND=r8)             :: fi, pi, sqd, t, tanfi

    sqd=SQRT(-delta)
    IF(ABS(q) <= 2.0_r8*eps*sqd) THEN
       fi=ATAN(one)*2.0_r8
    ELSE
       tanfi=ABS(2.0_r8*sqd/q)
       fi=ATAN(tanfi)
    END IF
  ...
  SUBROUTINE choose(x1,x2,x3,xmin,v0,v1,v2,m,root1,root2,proot2)
    ...
    REAL (KIND=r8), INTENT(IN)   :: x3
    REAL (KIND=r8), INTENT(IN)   :: xmin
    REAL (KIND=r8), INTENT(IN)   :: v0(:)
    REAL (KIND=r8), INTENT(IN)   :: v1(:)
    REAL (KIND=r8), INTENT(IN)   :: v2(:)
    INTEGER, INTENT(IN)     :: m
    REAL (KIND=r8), INTENT(OUT)  :: root1
    REAL (KIND=r8), INTENT(OUT)  :: root2
    REAL (KIND=r8), INTENT(OUT)  :: proot2
  ...
  FUNCTION pol3(scv0v1,scv0v2,v1nrm,scv1v2,v2nrm,x) RESULT(fn_val)
    ...
    fn_val=scv0v1 + x*v1nrm*v1nrm + 2.0_r8*x*scv0v2 + x**2*3.0_r8*scv1v2 +  &
         x**3*v2nrm*v2nrm*2.0_r8
  ...
  SUBROUTINE fsumsq(xold,p,n,alfk,xnew,fnew,m,ffunc,ctrl,fn_val)
    ...
    REAL (KIND=r8), INTENT(OUT)   :: fn_val
    ...
    IF(-1 == lctrl) fn_val=0.5_r8*dnrm2(m,fnew,1)**2
    !      write(10,*) 'FSUMSQ when ctrl=-1', fsumsq
    IF(lctrl < -10) ctrl=lctrl
    RETURN

20  IF(lctrl == 1) fn_val=0.5_r8*dnrm2(m,fnew,1)**2
    !      write(10,*) 'FSUMSQ when ctrl= 1', fsumsq
    ctrl=lctrl
...
240       a(k,j) = a(k,j)/work(k)
  ...
  SUBROUTINE sqrdc(x,n,p,qraux,jpvt,job)
    ...
    REAL (KIND=r8) :: maxnrm,tt
    REAL (KIND=r8) :: nrmxl,t,work(p)
    LOGICAL   :: negj,swapj
...
120    qraux(l) = zero
       ...
       DO  j = lp1, p
          ...
          tt = MAX(tt,zero)
          t = tt
          tt = one + 0.05_r8*tt*(qraux(j)/work(j))**2
          IF (tt /= one) THEN
  ...
  SUBROUTINE sqrsl(x,n,k,qraux,y,qy,qty,b,rsd,xb,job,info)
    ...
    INTEGER, INTENT(IN)        :: n
    INTEGER, INTENT(IN)        :: k
    REAL (KIND=r8), INTENT(IN)      :: qraux(:)
    REAL (KIND=r8), INTENT(IN)      :: y(:)
    REAL (KIND=r8), INTENT(OUT)     :: qy(:)
    REAL (KIND=r8), INTENT(OUT)     :: qty(:)
    REAL (KIND=r8), INTENT(OUT)     :: b(:)
    REAL (KIND=r8), INTENT(OUT)     :: rsd(:)
    REAL (KIND=r8), INTENT(OUT)     :: xb(:)
    INTEGER, INTENT(IN)        :: job
    INTEGER, INTENT(OUT)       :: info
...
240 RETURN
  ...
  FUNCTION dnrm2 ( n, x, incx) RESULT(fn_val)
    ...
    ! Local variables
    INTEGER    :: i, ix, j, next
    REAL (KIND=r8)  :: cuthi, cutlo, hitest, sum, xmax
...
200 ix = ix + 1
  ...
  SUBROUTINE dswap (n, x, incx, y, incy)
    ...
    REAL (KIND=r8), INTENT(IN OUT) :: x(:), y(:)
...
END MODULE OMSAO_elsunc_fitting_module
